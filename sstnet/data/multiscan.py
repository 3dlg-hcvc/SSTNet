...
class MultiScanInst(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple:
        if "test" in self.task:
            ...
            scene = self.files[index][0]
        xyz_origin -= xyz_origin.mean(axis=0)
        if not self.use_normals:
            vertex_normal = None
        ...
        if self.use_normals:
            ...
            feat = torch.cat((feat, vertex_normal), 1).float()
    ...
    def crop(self, xyz: np.ndarray, semantic_label: np.ndarray, instance_label: np.ndarray) -> Union[
        ...
        while True:  # HACK!
            ...
            while (valid_idxs.sum() > self.max_npoint):
                ...
                full_scale[:2] -= 32
            if valid_idxs.sum() > 5000 and np.any(semantic_label[valid_idxs] != self.ignore_label) and np.any(
